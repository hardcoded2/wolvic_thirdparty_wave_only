...
/*****************************************************************************
 * q_dot : to calcuate the rst of q1 dot d2
 * ***************************************************************************/
double q_dot(const q_type q1, const q_type q2)
{
    return ( q1[Q_X] * q2[Q_X] + q1[Q_Y] * q2[Q_Y] + q1[Q_Z] * q2[Q_Z] + q1[Q_W] * q2[Q_W]);
}

/*****************************************************************************
 * q_angle : to calcuate the angle between q1 and q2
 * ***************************************************************************/
double q_angle(const q_type q1, const q_type q2)
{
    q_type q2inv,res;
    q_invert(q2inv, q2);
    q_mult(res, q1, q2inv);
    return acosf(res[Q_W]) * 2.0;
}

void q_lerp(q_type destQuat, const q_type  startQuat, const q_type endQuat, double t)
{
    if(t < 0.0) t = 0.0;
    if(t > 1.0) t = 1.0;
    destQuat[Q_X] = startQuat[Q_X] + (endQuat[Q_X] - startQuat[Q_X]) * t;
    destQuat[Q_Y] = startQuat[Q_Y] + (endQuat[Q_Y] - startQuat[Q_Y]) * t;
    destQuat[Q_Z] = startQuat[Q_Z] + (endQuat[Q_Z] - startQuat[Q_Z]) * t;
    destQuat[Q_W] = startQuat[Q_W] + (endQuat[Q_W] - startQuat[Q_W]) * t;
